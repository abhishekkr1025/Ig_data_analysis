...
SELECT 
    ...
    (tableA.total_A / (SELECT COUNT(*) FROM users)) * 100 AS 'Percentage of Users who never commented',
    tableB.total_B AS 'Number of Users who commented on every photo',
    (tableB.total_B / (SELECT COUNT(*) FROM users)) * 100 AS 'Percentage of Users who commented on every photo'
FROM
    (
